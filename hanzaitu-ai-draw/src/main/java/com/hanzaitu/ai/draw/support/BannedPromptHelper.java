...
public class BannedPromptHelper {
	...
	public BannedPromptHelper() {
		...
		if (file.exists()) {
			...
			URL resource = BannedPromptHelper.class.getResource("/config/banned-words.txt");
	...
	public boolean isBanned(String promptEn) {
		...
		return this.bannedWords.stream().anyMatch(bannedWord ->
				Pattern.compile("\\b" + bannedWord + "\\b").matcher(finalPromptEn).find());
	}
