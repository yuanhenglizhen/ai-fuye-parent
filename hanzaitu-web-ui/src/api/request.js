...
service.interceptors.request.use(
  config => {
    ...
    const token = window.localStorage.getItem('token') || window.localStorage.getItem('App-Token')
    if (token && !isToken) {
      config.headers['token'] = token // 让每个请求携带自定义token 请根据实际情况自行修改
      config.headers['Access-Token'] = token // 让每个请求携带自定义token 请根据实际情况自行修改
      config.headers['Access-Token-Hzt'] = token // 让每个请求携带自定义token 请根据实际情况自行修改
    }
      config.headers['Hzt-Origin'] = window.location.origin;
      // config.headers['Hzt-Origin'] = 'http://45.14.106.237:6679';
    //
